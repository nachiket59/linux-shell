...
int process_exists(int *arr, int proc, int n){
	int i;
	for(i = 0; i<=n ;i++){
		if(arr[i] == proc){
			return 1;
...
int handle_fg(char* pid){
	int p = atoi(pid);
	if(process_exists(&children.stopped, p, children.cur_stopped)){
		kill(p,SIGCONT);
		children.cur_running ++;
		children.running[children.cur_running] = children.stopped[children.cur_stopped];
...
int handle_bg(char* pid){
	...
	if(process_exists(&children.stopped, p, children.cur_stopped)){
		kill(p,SIGCONT);
		children.cur_bg ++;
		children.bg[children.cur_bg] = children.stopped[children.cur_stopped];
		children.cur_stopped --;
		//printf("%d fg \n",children.cur_stopped );
		return 1;
	}
